//TCP ITERATIVE CHAT CLIENT PROGRAM
...
char serv_ip[] = "192.168.24.42"; /*server IP address*/
...
int main()
{
    ...
    printf("\nCLIENT: Connected to server %s:%d\n", serv_ip, serv_port);
    printf("CLIENT: Chat session started. Type 'stop' to end session.\n");
    ...
    while(1)
    {
        ...
        if((w = write(skfd, client_msg, strlen(client_msg))) < 0)
        {
        ...
        if((r = read(skfd, buff, sizeof(buff)-1)) <= 0)
        {
            printf("\nCLIENT: Server disconnected or error in receiving message\n");
            break;
        }
